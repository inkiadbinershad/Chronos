...
async function startServer() {
  ...
  // Seed only if in-memory and no products exist yet
  if (useMemory) {
    ...
    if (count === 0) {
      ...
      const admin = new User({ name: 'CHRONOS Admin', email: 'admin@chronos.com', password: 'admin123', role: 'admin' });
      await admin.save();
      console.log('Seeded in-memory database');
    }
  }
